...
4. **Error state = focus state** — ambos usam `ring-focus-gold`
5. **Queries duplicadas** — TopBar busca produtos só pra contar lowStock
6. **Componentes muito grandes** — InventoryTable ~200 linhas
7. **Constantes duplicadas** — `PRICE_CENTS_MULTIPLIER` em vários arquivos
8. **Botões sem `type`** — podem causar submit acidental em forms

### ✅ Problemas Técnicos Resolvidos

- **Hex cru em gráficos** — `SalesByChannelChart` e `SizesDonutChart` agora usam `CHART_SERIES_COLORS`, `CHART_AXIS_COLOR` e `CHART_TOOLTIP_STYLE` de `constants/chartTokens.ts`
- **Hex cru no Select** — SVG data URI do chevron referencia `ON_SURFACE_VARIANT_COLOR` em vez de `#d0c5af` literal

---

## 📊 Status por Módulo (vs Documento de Requisitos)

### Módulo 1 — Gestão de Estoque _(Resolve Dor 1)_
...
### Módulo 2 — Entradas de Mercadoria _(Resolve Dor 1)_
...
### Módulo 3 — Saídas e Vendas _(Resolve Dor 1 e 4)_
...
### Módulo 4 — Dashboard e Insights _(Resolve Dor 2 e 4)_
...
### Módulo 5 — Alertas Inteligentes _(Resolve Dor 1)_
...
### Módulo 6 — Clientes _(Resolve Dor 3)_
...
### Módulo 8 — Automação WhatsApp _(Opcional)_
...
3. **Separar error/focus states** — Cores diferentes
4. **Otimizar queries** — Evitar duplicação (TopBar)
5. **Adicionar `type="button"`** em todos os botões não-submit
