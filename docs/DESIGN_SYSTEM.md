...
Recharts renderiza SVG e não suporta CSS custom properties em `stroke`/`fill`. Use sempre as constantes de `src/constants/chartTokens.ts` — elas mapeiam os tokens do design system para hex resolvidos.

| Constante                         | Valor                  | Token de origem                              |
| --------------------------------- | ---------------------- | -------------------------------------------- |
| `CHART_PALETTE.primary`           | #f2ca50                | `--color-primary`                            |
| `CHART_PALETTE.primaryContainer`  | #d4af37                | `--color-primary-container`                  |
| `CHART_PALETTE.tertiary`          | #bfcdff                | `--color-tertiary`                           |
| `CHART_PALETTE.tertiaryContainer` | #97b0ff                | `--color-tertiary-container`                 |
| `CHART_PALETTE.primaryFixedDim`   | #e9c349                | `--color-primary-fixed-dim`                  |
| `CHART_SERIES_COLORS`             | array sequencial       | série de dados (donut, bar, line)            |
| `CHART_AXIS_COLOR`                | #d0c5af                | `--color-on-surface-variant`                 |
| `CHART_GRID_COLOR`                | rgba(208,197,175,0.08) | on-surface-variant @ 8%                      |
| `CHART_TOOLTIP_STYLE`             | objeto inline-style    | surface-bright + on-surface                  |
| `ON_SURFACE_VARIANT_COLOR`        | #d0c5af                | `--color-on-surface-variant` (SVG data URIs) |

> Se a paleta mudar em `globals.css`, atualize `chartTokens.ts` também.
